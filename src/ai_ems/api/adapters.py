...
def _candidate_response(
    ...
    operator_status = whole.get(
        "operator_strategy_status"
    )

    return RedispatchCandidateResponse(
        rank=candidate["rank"],
        ac_validation_rank=candidate.get(
            "ac_validation_rank"
        ),
        up_generator_id=redispatch[
            "up_generator_id"
        ],
        down_generator_id=redispatch[
            "down_generator_id"
        ],
        delta_mw=redispatch["delta_mw"],
        predicted_active_power_change_mw=(
            prediction.get(
                "active_power_change_mw"
            )
        ),
        ...
        apparent_power_before_mva=post.get(
            "apparent_power_mva"
        ),
        apparent_power_after_mva=after.get(
            "apparent_power_mva"
        ),
        improvement_mva=validation.get(
            "improvement_mva"
        ),
        loading_before_percent=validation.get(
            "loading_before_percent"
        ),
        loading_after_percent=validation.get(
            "loading_after_percent"
        ),
        violation_remaining=validation.get(
            "violation_remaining"
        ),
        whole_network_converged=(
            operator_status == "CONVERGED"
        ),
        new_violation_detected=whole.get(
            "new_violation_detected",
            False,
        ),
        violated_equipment_count_after=whole.get(
...
def to_contingency_response(
    ...
    return ContingencyResponseResponse(
        outage_line_id=result[
            "outage_line_id"
        ],
        target_line_id=result.get(
            "monitored_line_id"
        ),
